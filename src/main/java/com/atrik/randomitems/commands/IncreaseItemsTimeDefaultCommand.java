...
public class IncreaseItemsTimeDefaultCommand {
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("ri_increase_def_items_time")
    ...
    private static int execute(CommandContext<CommandSourceStack> ctx, int time) {
        ...
        GameManager.getGameManager().mainConfig.set("default_items_time", ((double) time) + (double) GameManager.getGameManager().mainConfig.getOrThrow("default_items_time"));
        return 1;
    }
}
