...
public class DecreaseItemsTimeDefaultCommand {
    ...
    private static int execute(CommandContext<CommandSourceStack> ctx, int time) {
        ...
        try {
            ...
            ctx.getSource().sendFailure(Component.translatable("commands.decrease_default_time.failure"));
            RandomItemsMod.getLogger().severe("Failed to increase def delay!");
            e.printStackTrace();
        }
        ctx.getSource().sendSuccess(() -> Component.translatable("commands.decrease_default_time.success", time, newTime), true);
